...
namespace eBooks.MessageHandlers
{
    public class PaymentCompletedHandler : BaseMessageHandler<PaymentCompleted>, IMessageHandler<PaymentCompleted>
    {
        ...
        public async Task SendEmail(PaymentCompleted message)
        {
            ...
            var notificationMessage = $"Payment {message.Purchase.PaymentStatus}.<br>Purchased book: {message.Purchase.Book.Title}<br>Total price: {message.Purchase.TotalPrice * 100}";
        ...
        public async Task NotifyUser(PaymentCompleted message)
        {
            var notification = new Notification
            {
                ...
                Message = $"Payment {message.Purchase.PaymentStatus}. Purchased book: {message.Purchase.Book.Title}. Total price: {message.Purchase.TotalPrice * 100}"
            };
